...
static const struct argp argp = {
    .options = opts,
    .parser = parse_arg,
    .doc = "Trace I/O amplification in storage systems using eBPF",
};
...
static int handle_event(void *ctx, void *data, size_t data_sz) {
  ...
  if (env.json_output) {
    ...
    fprintf(output_fp,
            "%s.%09llu %-8s %-12s %-8u %-8u %-15s %-8llu %-12llu %8.2f %u\n",
...
static void print_header() {
  ...
  fprintf(output_fp, "%-23s %-8s %-12s %-8s %-8s %-15s %-8s %-12s %8s %s\n",
          "TIME", "SYSTEM", "EVENT_TYPE", "PID", "TID", "COMM", "SIZE",
          "OFFSET", "LAT(us)", "RET");
  fprintf(output_fp, "%s\n",
          "===================================================================="
          "============");
...
static void print_summary() {
  if (env.json_output) {
    ...
    for (int i = 1; i < 6; i++) { // Skip UNKNOWN
      ...
      if (s->syscall_reads + s->syscall_writes + s->vfs_reads + s->vfs_writes +
      ...
      fprintf(output_fp, "    \"read_amplification\":%.2f,\n", read_amp);
      fprintf(output_fp, "    \"write_amplification\":%.2f\n", write_amp);
    ...
    fprintf(output_fp, "%-12s %10s %10s %10s %10s %10s %10s %12s %12s\n",
    ...
    for (int i = 1; i < 6; i++) { // Skip UNKNOWN
      ...
      fprintf(output_fp,
              "%-12s %10llu %10llu %10llu %10llu %10llu %10llu %12.2f %12.2f\n",
...
int main(int argc, char **argv) {
  ...
  if (env.verbose)
    fprintf(stderr,
            "Successfully started! Please run some storage workloads.\n");
...
// Helper function to increase RLIMIT_MEMLOCK
